...
import { AlertTriangle, Building2, CalendarCheck2, CircleDollarSign, Home, ShieldCheck, Users } from "lucide-react";
...
export default async function AdminPage() {
  ...
  const [users, pendingProperties, publishedProperties, agencies, unverifiedAgencies, bookings, payments, reports, myProperties] = await Promise.all([
    prisma.user.count(),
    ...
    <main className="min-h-screen bg-sand px-4 py-8 text-ink sm:px-6 sm:py-12">
      <div className="mx-auto max-w-7xl">
        <section className="overflow-hidden rounded-[2rem] bg-ink p-6 text-white shadow-soft sm:p-9">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-white/65">Bienvenue, {user.name || "Administrateur"}. Supervisez la plateforme, modérez les annonces et gérez les comptes depuis un espace dédié.</p>
            </div>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-2">
          <div className="rounded-[1.5rem] bg-white p-6 shadow-sm"><h2 className="text-xl font-extrabold">Actions rapides</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><Link href="/admin/annonces" className="rounded-2xl bg-forest px-4 py-3 text-center text-sm font-bold text-white">Modérer les annonces</Link><Link href="/admin/signalements" className="rounded-2xl border border-ink/10 px-4 py-3 text-center text-sm font-bold">Voir les signalements</Link><Link href="/admin/utilisateurs" className="rounded-2xl border border-ink/10 px-4 py-3 text-center text-sm font-bold">Gérer les utilisateurs</Link><Link href="/admin/agences" className="rounded-2xl border border-ink/10 px-4 py-3 text-center text-sm font-bold">Vérifier les agences</Link></div></div>
