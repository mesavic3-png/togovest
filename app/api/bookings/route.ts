...
export async function POST(request: Request) {
  try {
    ...
    const property = await prisma.property.findFirst({ where: { id: propertyId, status: "PUBLISHED", transactionType: "SHORT_TERM" } });
    ...
    if (property.minNights && nights < property.minNights) return NextResponse.json({ error: `Séjour minimum : ${property.minNights} nuit(s).` }, { status: 400 });
    if (property.maxGuests && guests > property.maxGuests) return NextResponse.json({ error: `Maximum ${property.maxGuests} voyageur(s).` }, { status: 400 });

    const conflict = await prisma.booking.findFirst({
      where: { propertyId, status: { in: ["PENDING", "CONFIRMED"] }, checkIn: { lt: checkOut }, checkOut: { gt: checkIn } },
      select: { id: true },
    });
    ...
    const booking = await prisma.booking.create({
      data: { propertyId, userId, checkIn, checkOut, guests, nights, totalAmount, currency: property.currency, guestName, guestPhone, notes },
    });
