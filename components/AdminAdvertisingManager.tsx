...
export function AdminAdvertisingManager({advertisements,requests}:{advertisements:Ad[];requests:AdRequest[]}){
  ...
  async function create(e:FormEvent<HTMLFormElement>){
    e.preventDefault(); setBusy("create"); setMessage(null); const f=new FormData(e.currentTarget); const payload=Object.fromEntries(f.entries());
    ...
    if(!r.ok){setMessage(j.error||"Création impossible.");return;} e.currentTarget.reset(); setMessage("Campagne créée."); router.refresh();
  }
  return <div className="space-y-10">
    <section className="rounded-[2rem] bg-white p-6 shadow-soft sm:p-8"><h2 className="text-2xl font-extrabold">Créer une campagne</h2><p className="mt-1 text-sm text-ink/55">Définissez l’annonceur, la destination, l’emplacement et la période.</p>
      <form onSubmit={create} className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <input name="destinationUrl" required type="url" className={input} placeholder="https://exemple.com"/><input name="imageUrl" type="url" className={input} placeholder="URL du visuel (optionnel)"/>
        ...
        <textarea name="description" rows={3} className={`${input} md:col-span-2`} placeholder="Description optionnelle"/><button disabled={busy==="create"} className="rounded-full bg-forest px-6 py-3 font-bold text-white md:col-span-2 disabled:opacity-50">Créer la campagne</button>
